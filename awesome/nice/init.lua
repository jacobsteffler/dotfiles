...
local debug = ret and helpers.debug or function()
end
local pairs = pairs
local ipairs = ipairs
-- > Awesome WM LIBS
...
local function create_button_image(name, is_focused, event, is_on)
    ...
    if not _private[key_color] then
        ...
        button_color = (event == "hover") and colors.lighten(button_color, 25) or (event == "press") and
                           colors.darken(button_color, 25) or button_color
...
local function create_titlebar_button(c, name, button_callback, property)
    ...
    if _private.tooltips_enabled then
        local tooltip = atooltip {
            timer_function = function()
                return
                    _private.tooltip_messages[name .. (property and (c[property] and "_active" or "_inactive") or "")]
            end,
            delay_show = 0.5,
    ...
    button_img:connect_signal("button::press", function(_, _, _, mb)
        if mb ~= nice.MB_LEFT then
            return
        end
    ...
    button_img:connect_signal("button::release", function(_, _, _, mb)
        if mb ~= nice.MB_LEFT then
            return
        end
...
local function get_titlebar_mouse_bindings(c)
    ...
    local buttons = gears.table.join(abutton({}, _private.mb_move, function()
    ...
    if _private.win_shade_enabled then
        buttons = gears.table.join(buttons, abutton({}, _private.mb_win_shade_rollup, function()
            _private.shade_roll_up(c)
        end), abutton({}, _private.mb_win_shade_rolldown, function()
            _private.shade_roll_down(c)
        end))
    end
    return buttons
...
local function create_titlebar_title(c)
    ...
    local function update()
        local text_color = is_contrast_acceptable(title_color_light, client_color) and title_color_light or
                               title_color_dark
        title_widget.markup = ("<span foreground='%s' font='%s'>%s</span>"):format(text_color, _private.titlebar_font,
                                  c.name)
    end
...
function _private.add_window_decorations(c)
    ...
    local background_fill_top = gradient(lighten(titlebar_gradient_c1_lighten), client_color, titlebar_height, 0,
    ...
    local corner_bottom_left_img = shapes.flip(create_corner_top_left {
        ...
        stroke_offset_inner = 1.5,
        stroke_offset_outer = 0.5,
        stroke_source_outer = gradient(stroke_color_outer_bottom, stroke_color_outer_sides, bottom_edge_height, 0, 0.25),
...
local function validate_mb_bindings()
    local action_mbs = {"mb_move", "mb_contextmenu", "mb_resize", "mb_win_shade_rollup", "mb_win_shade_rolldown"}
